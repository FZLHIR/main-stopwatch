 ...
 *  STATIC PROTOTYPES
 **********************/
static void lv_100ask_page_manager_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);//创建对象时调用
static void lv_100ask_page_manager_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj);//销毁对象时调用
static void lv_100ask_page_manager_event(const lv_obj_class_t * class_p, lv_event_t * e);//事件处理函数

static void lv_100ask_page_manager_page_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);//创建页面时调用
static void lv_100ask_page_manager_page_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj);//销毁页面时调用
static void lv_page_manager_load_page_event_cb(lv_event_t * e);//加载页面事件回调函数
static void lv_100ask_page_manager_page_del_event_cb(lv_event_t * e);//删除页面事件回调函数

static void page_close_anim_timeline_create(lv_anim_timeline_t * at, const lv_anim_timeline_wrapper_t * wrapper);//创建打开页面动画时间线
static void page_open_anim_timeline_create(lv_anim_timeline_t * at, const lv_anim_timeline_wrapper_t * wrapper);//创建关闭页面动画时间线

#if LV_100ASK_PAGE_MANAGER_CUSTOM_ANIMARION == 0
    static void default_open_page(lv_obj_t * obj);//默认打开页面动画
    static void default_close_page(lv_obj_t * obj);//默认关闭页面动画
#endif

static void lv_obj_100ask_open_page_anim_start_cb(lv_anim_t * a);//打开页面动画开始回调函数
static void lv_obj_100ask_open_page_anim_ready_cb(lv_anim_t * a);//打开页面动画准备好回调函数
static lv_obj_t * get_page(lv_obj_t * page_manager, char *name);//获取页面对象
static char * get_page_name(lv_obj_t * page_manager, lv_obj_t * page);//获取页面名称
